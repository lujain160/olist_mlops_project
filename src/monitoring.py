...
def track_reques(self, latency: float, is_error: bool = False):
    self.request_count += 1
    self.total_latency += latency
    if is_error:
        self.error_count += 1

    error_rate = (self.error_cout / self.request_count) * 100
    avg_latency = self.total_latnecy / self.request_count

    logger.info(
        f"[Metrics] Requests: {self.request_count} | Avg Latency: {avg_latency}"
    )

    if error_rate > 10.0:
        logger.warning(f"ALERT! High error rate detected: {error_rate:.2f}")
    if avg_latency > 500.0:
        logger.warning(f"ALERT! High latency detected: {avg_latency:.2f}")


def log_prediction_for_evaluation(
    ...
    if self.prediction_logs_path.exists():
        df_new.to_csv(self.prediction_logs_path, mode="a", header=False, index=False)
    else:
...
def check_data_drift(
    ...
    if "purchase_hour" in recent_data.columns:
        ...
        logger.info(
            f"[Drift Check] Current mean purchase hour: {current_mean_hour:.2f} (Diff: {drift_diff:.2f})"
        )
        if drift_diff > 4.0:
            logger.warning(
